...
import httplib2
import subprocess
from userbot.modules.download import progress, humanbytes, time_formatter
...
async def _(event):
    ...
    http = authorize(G_DRIVE_TOKEN_FILE, storage)
    # Authorize, get file parameters, upload file and print out result URL for download
    await mone.edit(f"Searching for {input_str} in your Google Drive ...")
...
async def DoTeskWithDir(http, input_directory, event, parent_id):
    ...
    for a_c_f_name in list_dirs:
        ...
        if os.path.isdir(current_file_name):
            ...
            g_drive_link = await upload_file(http, current_file_name, file_name, mime_type, event)
            r_p_id = parent_id
...
async def gdrive_search(http, search_query):
    if parent_id:
        query = "'{}' in parents and (title contains '{}')".format(parent_id, search_query)
    else:
        ...
        try:
            ...
            for file in response.get("items",[]):
                ...
                if file.get("mimeType") == G_DRIVE_DIR_MIME_TYPE:
                    msg += f"➺ <a href='https://drive.google.com/drive/folders/{file_id}'>{file_title}</a> (folder)\n"
                else:
